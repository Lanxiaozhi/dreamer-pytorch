...
CONTROL_SUITE_ENVS = ['cartpole-balance', 'cartpole-swingup', 'reacher-easy', 'finger-spin', 'cheetah-run', 'ball_in_cup-catch',
                      'walker-walk', 'reacher-hard', 'walker-run', 'hopper-hop', 'humanoid-stand', 'humanoid-walk', 'fish-swim', 'acrobot-swingup']
CONTROL_SUITE_ACTION_REPEATS = {'cartpole': 8, 'reacher': 4, 'finger': 2, 'cheetah': 4,
                                'ball_in_cup': 6, 'walker': 2, 'humanoid': 2, 'fish': 2, 'acrobot': 4, 'hopper': 2}
...
def postprocess_observation(observation, bit_depth):
    ...
    return torch.clamp(torch.floor((observation + 0.5) * 2 ** bit_depth) * 2 ** (8 - bit_depth), 0, 2 ** 8 - 1).to(torch.uint8)


def _images_to_observation(images, bit_depth, device):
    images = torch.tensor(cv2.resize(images, (64, 64), interpolation=cv2.INTER_LINEAR).transpose(
        2, 0, 1), dtype=torch.float32, device=device)  # Resize and put channel first
...
class ControlSuiteEnv():
    def __init__(self, env, symbolic, seed, max_episode_length, action_repeat, bit_depth, device):
    ...
    def reset(self):
        ...
        if self.symbolic:
            ...
            return _images_to_observation(self._env.physics.render(camera_id=0), self.bit_depth, self.device)
    ...
    def step(self, action):
        ...
        if self.symbolic:
            ...
            observation = _images_to_observation(
                self._env.physics.render(camera_id=0), self.bit_depth, self.device)
...
def Env(env, symbolic, seed, max_episode_length, action_repeat, bit_depth, device):
    if env in GYM_ENVS:
        raise BaseException("No device info")
        return GymEnv(env, symbolic, seed, max_episode_length, action_repeat, bit_depth)
    elif env in CONTROL_SUITE_ENVS:
        return ControlSuiteEnv(env, symbolic, seed, max_episode_length, action_repeat, bit_depth, device)
...
class EnvBatcher():
    ...
    def step(self, actions):
        ...
        observations, rewards, dones = torch.cat(observations), torch.tensor(
            rewards, dtype=torch.float32), torch.tensor(dones, dtype=torch.uint8)
