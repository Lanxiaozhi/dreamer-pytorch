...
env = Env(args.env, args.symbolic_env, args.seed, args.max_episode_length,
          args.action_repeat, args.bit_depth, args.device)
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1, desc='Episodes'):
    # Model fitting
    losses = []
    ...
    # print("training loop")
    for s in tqdm(range(args.collect_interval), desc="Training Loop"):
    ...
    with torch.no_grad():
        ...
        pbar = tqdm(range(args.max_episode_length //
                    args.action_repeat), desc='Data Collection')
        for t in pbar:
            # print("step",t)
    ...
    if episode % args.test_interval == 0:
        ...
        test_envs = EnvBatcher(Env, (args.env, args.symbolic_env, args.seed, args.max_episode_length,
                               args.action_repeat, args.bit_depth, args.device), {}, args.test_episodes)
        ...
        with torch.no_grad():
            ...
            for t in pbar:
                ...
                if not args.symbolic_env:  # Collect real vs. predicted frames for video
                    video_frames.append(make_grid(torch.cat([observation.cpu(), observation_model(
        ...
        writer.add_scalar("test_reward", sum(
            metrics['test_rewards'][-1])/len(metrics['test_rewards'][-1]), episode)
    ...
    writer.add_scalar("observation_loss",
                      metrics['observation_loss'][-1][-1], metrics['steps'][-1])
    writer.add_scalar(
        "reward_loss", metrics['reward_loss'][-1][-1], metrics['steps'][-1])
    writer.add_scalar(
        "kl_loss", metrics['kl_loss'][-1][-1], metrics['steps'][-1])
    writer.add_scalar(
        "actor_loss", metrics['actor_loss'][-1][-1], metrics['steps'][-1])
    writer.add_scalar(
        "value_loss", metrics['value_loss'][-1][-1], metrics['steps'][-1])
