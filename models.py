...
def bottle(f, x_tuple, feature=False):
    ...
    if not feature:
        ...
        y, y_f = f(*map(lambda x: x[0].view(x[1][0] * x[1]
                                            [1], *x[1][2:]), zip(x_tuple, x_sizes)), feature=feature)
        y_size = y.size()
        output = y.view(x_sizes[0][0], x_sizes[0][1], *y_size[1:])
        y_f_size = y_f.size()
        # output_f = y_f.view(x_sizes[0][0], x_sizes[0][1], *y_f_size[1:])
        output_f = y_f
...
class TransitionModel(jit.ScriptModule):
    ...
    def __init__(self, belief_size, state_size, action_size, hidden_size, embedding_size, activation_function='relu', min_std_dev=0.1):
        ...
        self.modules = [self.fc_embed_state_action, self.fc_embed_belief_prior,
                        self.fc_state_prior, self.fc_embed_belief_posterior, self.fc_state_posterior]
    ...
    # s : -x--X--X--X--X--X-
    @jit.script_method
    def forward(self, prev_state: torch.Tensor, actions: torch.Tensor, prev_belief: torch.Tensor, observations: Optional[torch.Tensor] = None, nonterminals: Optional[torch.Tensor] = None, augment: bool = False) -> List[torch.Tensor]:
        '''
        ...
        T = actions.size(0) + 1
        beliefs, prior_states, prior_means, prior_std_devs, posterior_states, posterior_means, posterior_std_devs = [torch.empty(
            0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T
        posterior_states_k = [torch.empty(0)] * T
        ...
        for t in range(T - 1):
            ...
            if observations is not None:
                ...
                if augment: # generate another posterior state
                    posterior_states_k[t + 1] = posterior_means[t + 1] + \
                        posterior_std_devs[t + 1] * \
                        torch.randn_like(posterior_means[t + 1])
                else:
                    posterior_states_k[t + 1] = posterior_states_k[t]
        ...
        if observations is not None:
            if augment:
                hidden += [torch.stack(posterior_states[1:], dim=0), torch.stack(
                    posterior_means[1:], dim=0), torch.stack(posterior_std_devs[1:], dim=0), torch.stack(posterior_states_k[1:], dim=0)]
            else:
                hidden += [torch.stack(posterior_states[1:], dim=0), torch.stack(
...
class RewardModel(jit.ScriptModule):
    ...
    # @jit.script_method
    def forward(self, belief, state, feature=False):
