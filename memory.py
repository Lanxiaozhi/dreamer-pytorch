...
class ExperienceReplay():
    ...
    def append(self, observation, action, reward, done):
        if self.symbolic_env:
            ...
            self.observations[self.idx] = postprocess_observation(observation, self.bit_depth).cpu(
            ).numpy()  # Decentre and discretise visual observations (to save memory)
    ...
    def _sample_idx(self, L):
        ...
        while not valid_idx:
            ...
            valid_idx = not ((idx < self.idx < idx + L)
                             or (idx < (self.idx + self.size) < idx + L))
            idxs = np.arange(idx, idx + L) % self.size
        return idxs
    ...
    def _retrieve_batch(self, idxs, n, L):
        ...
        observations = torch.as_tensor(
            self.observations[vec_idxs].astype(np.float32)).to(self.device)
